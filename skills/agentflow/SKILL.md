...
- The completion checker derives whether review is required from Git changed paths and current review evidence. A first Reply does not treat every tracked file as in-scope; it uses Git changes since the current Ask (or its parent commit) and still exempts first-time Agentflow bookkeeping. Exact current-Ask `skip-review: <accepted tradeoff>` skips only the final independent cross-check; every other completion and consequential-work gate remains active.
